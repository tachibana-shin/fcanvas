!(function () {
    ...
    PREFIX.forEach((prefix) => {
      if (window["on" + type.replace(/-/g, "") + type + "end"] !== undefined) {
        ["End", "Run", "Start", "Cancel"].forEach((after) => {
          fx[type + after] =
            prefix === ""
              ? type + after.toLowerCase()
              : prefix.replace(/-/g, "") + camelCase("-" + type) + after;
        });
        return (
          (fx[type + "Prop"] = prefix === "" ? type : camelCase(prefix + type)),
          false
        );
      }
    });
  ...
  const windowSizePropertyes = {
    ...
    windowHeight: {
      get: () =>
        window.innerHeight || DOMe.clientHeight || DOM.body.clientHeight,
    },
  };
  ...
  function isNumeric(n) {
    return (
      typeof n === "number" || (typeof n === "string" && !Number.isNaN(n - 0))
    );
  }
  ...
  function toRgbColor([red = 0, green = red, blue = green, alpha = 1]) {
    if (isLikeArray(red)) {
      ...
      return (red + "").match(/^#|[a-z]/i)
        ? red
        : `${colorMode._colorMode}a(${red}, ${green}, ${blue}, ${alpha})`;
    }
  }
  ...
  fnSpread(window, {
    ...
    disableAutoClear() {
      ENV.disableAutoClear = true;
    },
    createMatrix(css) {
      const { a, b, c, d, e, f } = new DOMatrix(css);
  ...
  const methods = {
    map: (a, b, c, d, e) => ((a - b) * (e - d)) / (c - b) + d,
    constrain: (value, min, max) => Math.min(Math.max(min, value), max),
  };
  ...
  function fontSize(size) {
    ...
    if (size === undefined) return parseFloat(_font.size);
    size = AutoToPx(size);
    font([_font.weight, size + "px", _font.family].join(" "));
    return parseFloat(size);
  }

  function fontFamily(name) {
    var _font = fontToArray();
    if (name === undefined) return _font.family;
  ...
  function fontWeight(type) {
    var _font = fontToArray();
    if (type === undefined) return _font.weight;
  ...
  fnSpread(methods, {
    ...
    ...(() => {
      ...
      for (
        ...
        result[propertyes[index]] = (...args) => {
          args.length &&
            (canvasLocal.context2d[propertyes[index] + "Style"] = toRgbColor(
              args
            ));
          canvasLocal.context2d[propertyes[index]]();
        };
        result[camelCase("get-" + propertyes[index])] = () =>
          canvasLocal.context2d[propertyes[index]];
      }
      return result;
    })(),
    ...
    shadowOffset(...args) {
      ...
      if (isNumeric(args[0])) context2d.shadowOffsetX = args[0];
      if (isNumeric(args[1])) context2d.shadowOffsetX = args[1];
    },
    ...
    ...(() => {
      ...
      for (const property in propertyes) {
        result[property] = (...args) =>
          canvasLocal.context2d[propertyes[property]](...args);
      }
    ...
    ...(() => {
      ...
      for (const keyword in propertyes) {
        result[keyword] = (...arg) =>
          canvasLocal.context2d[propertyes[keyword]](...args);
      }
      return result;
    })(),
    ...(() => {
      ...
      for (
        let index = 0,
          propertyes = ["transform", "setTransform"],
          length = propertyes.length;
        index < length;
        index++
      ) {
    ...
    ...(() => {
      ...
      for (
        let index = 0,
          propertyes = ["abs", "floor", "pow", "round", "sqrt", "PI", "ceil"],
          length = propertyes.length;
        index < length;
        index++
      ) {
    ...
    pie(x, y, r, d1, d2, a) {
      begin();
      move(x, y);
      canvasLocal.context2d.arc(
        x,
        y,
        r,
        getRadius(d1) - Math.PI / 2,
        getRadius(d2) - Math.PI / 2,
        a
      );
      to(x, y);
      close();
    ...
    arc: (c, t, e, i, n, o) => {
      ...
      canvasLocal.context2d.arc(
        ...
        t,
        e,
        getRadius(i) - Math.PI / 2,
        getRadius(n) - Math.PI / 2,
        o
      );
      close();
    },
    ...
    square: (x, y, w, ...radius) => rect(x, y, w, w, ...radius),
  });
  ...
  function handlerEvent(event) {
    try {
      window.touches = getTouchInfo(event.touches || [event]);
      window.changedTouches = getTouchInfo(event.changedTouches || [event]);
  ...
  function toArray(arr) {
    if (arr != null && typeof arr === "object" && "length" in arr) {
      return arr;
    } else {
      if (arr === undefined) {
        return [];
  ...
  async function handlerReady() {
    ...
    if (typeof preload === "function") {
      returnPreload = preload();

      if (Array.isArray(returnPreload)) {
        returnPreload = Promise.all(returnPreload);
      } else {
        returnPreload = Promise.all([returnPreload]);
      }

      returnPreload = await returnPreload;
    }

    const argv = toArray(
      typeof setup === "function" ? setup(...returnPreload) : []
    );
