...
export function AutoToPx(string, fi, fontSize) {
  if (typeof string == "string") {
    ...
    switch (dp) {
      ...
      case "vmin":
        return (
          (Math.min(windowSize.windowWidth, windowSize.windowHeight) * number) /
          100
        );
      case "vmax":
        return (
          (Math.max(windowSize.windowWidth, windowSize.windowHeight) * number) /
          100
        );
      case "%":
...
export async function toPromise(promise) {
